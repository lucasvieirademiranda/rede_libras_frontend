...
class Hands extends React.Component {
    ...
    state = {
        columnDefs: [
            { headerName: "Configuração de Mão", field: "name", editable: false, filter: "agTextColumnFilter", sortable: true, suppressMovable: true },
            { headerName: "Descrição", field: "description", editable: false, filter: "agTextColumnFilter", sortable: true, suppressMovable: true },
            { headerName: "", width: 100,  cellRenderer: function(event) { return buildControl('/hands/edit/' + event.data.id, 'Editar', edit); } },
            { headerName: "", width: 100,  cellRenderer: function(event) { return buildControl('/hands/delete/' + event.data.id, 'Remover', remove); } }
        ],
        rowData: [],
	...
	render() {
		return (
            <div>
                <Row>
                    <h3>Configurações de Mão</h3>
                </Row>
                <Row>
                    <Button style={ { float: 'right' } } waves='light' node='a' href='/hands/add'>Adicionar</Button>
                </Row>
                <Row>
